# Query Understanding 模块设计文档

> 统一的 LLM 驱动查询理解模块，替代原有的固定规则 Query Routing + Query Expansion

## 1. 背景与动机

### 1.1 现有架构的问题

当前系统采用两个独立模块处理用户查询：

```
用户查询 → [Query Routing] → [Query Expansion] → 检索
              (固定规则)        (LLM 扩展)
```

**Query Routing 的问题：**

| 问题 | 示例 | 影响 |
|------|------|------|
| 规则过于简单 | 只有 3 种路由类型 | 无法处理复杂意图 |
| 短查询一刀切 | "熊猫头" 被判为 Exact | 丢失语义理解 |
| 情绪词优先级混乱 | "猫咪无语" 被判为 Emotion | 忽略主体 |
| 网络梗匹配硬编码 | 词库需手动维护 | 无法跟上新梗 |
| 无法识别复合意图 | "熊猫头摆烂" | 只能单一分类 |

**Query Expansion 的问题：**

| 问题 | 描述 |
|------|------|
| 与 Routing 割裂 | 两个模块各自为战，无法协同 |
| 扩展可能偏离 | 不知道用户真实意图，盲目扩展 |
| 无关键词提取 | 只输出语义描述，BM25 无法利用 |
| 无策略建议 | 无法告知检索层如何平衡 Dense/Sparse |

### 1.2 目标

设计一个统一的 **Query Understanding** 模块：

1. **单次 LLM 调用** 完成意图识别、查询改写、关键词提取
2. **结构化输出** 直接指导检索策略
3. **细粒度意图** 支持 7+ 种查询意图

---

## 2. 整体架构

### 2.1 模块位置

```
┌─────────────────────────────────────────────────────────────────┐
│                         Search Service                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   用户查询                                                       │
│      │                                                          │
│      ▼                                                          │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │             Query Understanding Service                  │   │
│   │                   (本文档设计的模块)                       │   │
│   │                                                          │   │
│   │   ┌──────────┐   ┌──────────────────┐                   │   │
│   │   │   LLM    │ → │  Output Parser   │                   │   │
│   │   └──────────┘   └──────────────────┘                   │   │
│   │                                                          │   │
│   │   输出: QueryPlan (结构化)                                │   │
│   └─────────────────────────────────────────────────────────┘   │
│      │                                                          │
│      ▼                                                          │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │                  Hybrid Search                           │   │
│   │                                                          │   │
│   │   SemanticQuery ──→ Embedding ──→ Dense Search          │   │
│   │   Keywords ──────────────────────→ BM25 Search          │   │
│   │   Strategy ──────────────────────→ Fusion Weights       │   │
│   │                                                          │   │
│   └─────────────────────────────────────────────────────────┘   │
│      │                                                          │
│      ▼                                                          │
│   搜索结果                                                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 2.2 数据流（非流式）

```
Input:  "熊猫头无语"
           │
           ▼
    ┌──────────────┐
    │   LLM Call   │
    └──────┬───────┘
           │
           ▼
    ┌──────────────┐
    │ JSON Parse   │
    └──────┬───────┘
           │
           ▼
    ┌──────────────┐
    │  Validation  │ ──→ 修正异常值
    └──────┬───────┘
           │
           ▼
Output: QueryPlan {
          intent: "composite",
          semantic_query: "熊猫头表情包表达无语无奈...",
          keywords: ["熊猫头", "无语", "无奈"],
          strategy: { dense_weight: 0.6, ... },
          filters: { categories: ["熊猫头"] }
        }
```

### 2.3 流式输出架构（支持前端展示思考过程）

为了让用户在等待时看到 AI 的思考过程，我们采用 **先思考后输出** 的流式设计：

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      Query Understanding Service                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  用户查询: "熊猫头无语"                                                       │
│       │                                                                      │
│       ▼                                                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                    UnderstandStream()                                   │ │
│  │                                                                         │ │
│  │   HTTP POST ──→ LLM API (stream=true)                                  │ │
│  │        │                                                                │ │
│  │        ▼                                                                │ │
│  │   SSE 事件流 (Server-Sent Events):                                     │ │
│  │   ┌─────────────────────────────────────────────────────────────────┐  │ │
│  │   │                                                                 │  │ │
│  │   │  Phase 1: Thinking (流式发送给前端)                             │  │ │
│  │   │  ──────────────────────────────────────────────────             │  │ │
│  │   │  data: {"delta":{"content":"<think>"}}                          │  │ │
│  │   │  data: {"delta":{"content":"用户想找熊猫头类型的"}}              │  │ │
│  │   │  data: {"delta":{"content":"表情包，同时表达无语的情绪"}}         │  │ │
│  │   │  data: {"delta":{"content":"，这是一个复合意图..."}}             │  │ │
│  │   │  data: {"delta":{"content":"</think>"}}                         │  │ │
│  │   │       │                                                         │  │ │
│  │   │       ▼                                                         │  │ │
│  │   │  thinkingCh ← 实时推送给前端显示                                │  │ │
│  │   │                                                                 │  │ │
│  │   │  Phase 2: JSON Output (累积后解析)                              │  │ │
│  │   │  ──────────────────────────────────────────────────             │  │ │
│  │   │  data: {"delta":{"content":"{"}}                                │  │ │
│  │   │  data: {"delta":{"content":"\"intent\":"}}                      │  │ │
│  │   │  data: {"delta":{"content":"\"composite\","}}                   │  │ │
│  │   │  ...                                                            │  │ │
│  │   │  data: {"delta":{"content":"}"}}                                │  │ │
│  │   │       │                                                         │  │ │
│  │   │       ▼                                                         │  │ │
│  │   │  jsonBuffer ← 累积完整 JSON                                     │  │ │
│  │   │                                                                 │  │ │
│  │   │  data: [DONE]                                                   │  │ │
│  │   └─────────────────────────────────────────────────────────────────┘  │ │
│  │        │                                                                │ │
│  │        ▼                                                                │ │
│  │   JSON Parse ──→ QueryPlan                                             │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│        │                    │                                                │
│        │                    ▼                                                │
│        │              前端实时展示思考过程                                    │
│        ▼                                                                     │
│   返回 QueryPlan                                                             │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### 2.3.1 LLM 输出格式

LLM 被指导先输出思考过程（用 `<think>` 标签包裹），再输出 JSON：

```
<think>
用户查询"熊猫头无语"包含两个部分：
1. "熊猫头" - 这是表情包的主体类型，属于 subject 意图
2. "无语" - 这是情绪表达，属于 emotion 意图
综合来看这是一个复合意图（composite），需要同时匹配主体和情绪。
建议使用中等的 dense_weight (0.6)，既保留语义理解又兼顾关键词匹配。
</think>
{"intent":"composite","semantic_query":"熊猫头表情包，表达无语、无奈、嫌弃的情绪，黑白熊猫脸露出一脸嫌弃翻白眼的样子，对某事无话可说不想理会","keywords":["熊猫头","无语","无奈"],"synonyms":["嫌弃","翻白眼"],"strategy":{"dense_weight":0.6,"need_exact_match":false},"filters":{"categories":["熊猫头"]}}
```

#### 2.3.2 流式解析状态机

```
                    ┌──────────────┐
                    │    START     │
                    └──────┬───────┘
                           │ 收到 "<think>"
                           ▼
                    ┌──────────────┐
         ┌─────────│   THINKING   │◄────────┐
         │         └──────┬───────┘         │
         │ 发送 token     │                 │ 继续收集
         │ 到 thinkingCh  │ 收到 "</think>" │ thinking 内容
         └────────────────┤                 │
                          ▼                 │
                    ┌──────────────┐        │
                    │  AWAITING_   │        │
                    │    JSON      │────────┘
                    └──────┬───────┘   非 "{" 字符
                           │ 收到 "{"
                           ▼
                    ┌──────────────┐
         ┌─────────│ PARSING_JSON │◄────────┐
         │         └──────┬───────┘         │
         │ 累积到         │                 │ 继续累积
         │ jsonBuffer     │ JSON 完整       │
         └────────────────┤ (括号匹配)      │
                          ▼                 │
                    ┌──────────────┐        │
                    │   COMPLETE   │────────┘
                    └──────────────┘
```

#### 2.3.3 Go 接口设计

```go
// UnderstandProgress 表示流式理解的进度更新
type UnderstandProgress struct {
    Stage        string `json:"stage"`          // "thinking" | "parsing" | "done"
    ThinkingText string `json:"thinking_text"`  // 思考内容 (增量)
    IsDelta      bool   `json:"is_delta"`       // 是否为增量更新
}

// UnderstandStream 流式理解用户查询
// 参数:
//   - ctx: 上下文
//   - query: 用户原始查询
//   - progressCh: 进度更新通道（用于发送思考过程）
// 返回:
//   - *QueryPlan: 结构化的查询计划
//   - error: 错误信息
func (s *QueryUnderstandingService) UnderstandStream(
    ctx context.Context,
    query string,
    progressCh chan<- UnderstandProgress,
) (*QueryPlan, error)
```

#### 2.3.4 前端展示效果

```
┌─────────────────────────────────────────────────────────────┐
│  搜索: 熊猫头无语                                            │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  🤔 AI 正在理解...                                           │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ 用户查询"熊猫头无语"包含两个部分：                    │   │
│  │ 1. "熊猫头" - 这是表情包的主体类型                   │   │
│  │ 2. "无语" - 这是情绪表达                             │   │
│  │ 综合来看这是一个复合意图，需要同时匹配主体和情绪▌    │   │  ← 打字机效果
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ────────────────────────────────────────────────────────   │
│                                                             │
│  ✅ 理解完成                                                 │
│  📝 意图: 复合查询 (主体 + 情绪)                             │
│  🔑 关键词: 熊猫头, 无语, 无奈                               │
│                                                             │
│  🔍 正在搜索...                                              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 2.3.5 优势

| 方面 | 描述 |
|------|------|
| **用户体验** | 用户看到 AI 思考过程，减少等待焦虑 |
| **可解释性** | 展示为什么这样理解查询，增加信任 |
| **调试友好** | 开发者可以看到 LLM 的推理过程 |
| **灵活性** | thinking 内容可用于日志分析和模型优化 |

---

## 3. 核心数据结构

### 3.1 QueryPlan (输出结构)

```
QueryPlan
├── intent: QueryIntent           # 用户意图分类
├── semantic_query: string        # 语义改写 (用于 Dense)
├── keywords: []string            # 关键词列表 (用于 BM25)
├── synonyms: []string            # 同义词/相关词 (可选)
├── strategy: SearchStrategy      # 检索策略
│   ├── dense_weight: float32     # Dense 权重 (0-1)
│   ├── need_exact_match: bool    # 是否需要精确匹配
│   └── prefetch_multiplier: int  # Prefetch 倍数
├── filters: SuggestedFilters     # 建议的过滤条件 (可选)
│   ├── categories: []string
│   └── is_animated: *bool
└── reasoning: string             # LLM 推理过程 (调试用)
```

### 3.2 QueryIntent (意图枚举)

| Intent | 描述 | 典型查询 | 检索特点 |
|--------|------|----------|----------|
| `emotion` | 情绪表达 | 无语、开心、emo | 重语义，需理解情绪的多种表现 |
| `meme` | 网络流行梗 | 芭比Q、绝绝子 | 需同义词扩展，理解梗的含义 |
| `subject` | 主体/角色 | 熊猫头、猫咪 | 可加分类过滤，精确+语义结合 |
| `scene` | 使用场景 | 上班、恋爱 | 重语义，理解场景下的情绪 |
| `action` | 动作描述 | 比心、翻白眼 | 语义+关键词结合 |
| `text` | 文字内容 | 有666的、写着谢谢 | 重 BM25，搜索 OCR 文本 |
| `composite` | 复合意图 | 熊猫头无语 | 多意图平衡 |

### 3.3 SearchStrategy (检索策略)

```
dense_weight 含义:
  0.0 ─────────────────────────────────── 1.0
   │                                       │
  全 BM25                               全 Dense
  (精确匹配)                            (语义理解)

典型配置:
  - text 意图:      dense_weight = 0.3 (BM25 为主)
  - subject 意图:   dense_weight = 0.5 (均衡)
  - meme 意图:      dense_weight = 0.6 (略偏语义)
  - emotion 意图:   dense_weight = 0.8 (语义为主)
  - scene 意图:     dense_weight = 0.8 (语义为主)
```

---

## 4. LLM Prompt 设计

### 4.1 设计原则

1. **先思考后输出**: 使用 `<think>` 标签包裹推理过程，支持流式展示
2. **结构化输出**: 思考后输出 JSON，确保可解析
3. **Few-shot 示例**: 提供典型案例指导 LLM
4. **策略指南**: 明确不同意图的处理方式
5. **边界约束**: 限制输出长度和取值范围

### 4.2 Prompt 结构

```
[System Prompt]
├── 角色定义: 表情包搜索查询理解助手
├── 输出格式说明:
│   ├── 第一步: <think>思考过程</think>
│   └── 第二步: JSON 输出
├── 意图类型说明 (7 种意图的定义和示例)
├── JSON Schema 定义
├── 策略指南 (不同意图的 dense_weight 建议)
└── Few-shot 示例 (5-8 个典型案例，含思考过程)

[User Prompt]
└── 用户的原始查询
```

### 4.3 完整 Prompt 示例

```
你是表情包搜索查询理解助手。你的任务是理解用户的搜索意图，并生成结构化的查询计划。

【输出格式】
请严格按照以下格式输出：
1. 先用 <think></think> 标签包裹你的思考过程（2-4 句话，简洁明了）
2. 然后直接输出 JSON（不要用 markdown 代码块）

【意图类型】
- emotion: 情绪表达（无语、开心、emo）
- meme: 网络流行梗（芭比Q、绝绝子、yyds）
- subject: 主体/角色（熊猫头、猫咪、柴犬）
- scene: 使用场景（上班、恋爱、考试）
- action: 动作描述（比心、翻白眼、点赞）
- text: 文字内容（有666的、写着谢谢）
- composite: 复合意图（熊猫头无语 = subject + emotion）

【JSON Schema】
{
  "intent": "emotion|meme|subject|scene|action|text|composite",
  "semantic_query": "50-100字的语义描述，用于向量搜索",
  "keywords": ["关键词1", "关键词2"],  // 最多5个
  "synonyms": ["同义词1", "同义词2"],  // 最多5个，可选
  "strategy": {
    "dense_weight": 0.0-1.0,  // 0=全BM25, 1=全语义
    "need_exact_match": true/false
  },
  "filters": {  // 可选
    "categories": ["熊猫头"]
  }
}

【策略指南】
- text 意图: dense_weight = 0.3 (BM25 为主，搜索 OCR 文本)
- subject 意图: dense_weight = 0.5 (均衡)
- meme 意图: dense_weight = 0.6 (需要理解梗的含义)
- emotion/scene 意图: dense_weight = 0.8 (语义为主)
- composite 意图: dense_weight = 0.5-0.7 (根据具体情况)

【示例】

输入: 无语
<think>
用户想表达"无语"的情绪，这是典型的 emotion 意图。需要扩展相关情绪词（无奈、嫌弃、翻白眼），并描述这类表情包的视觉特征。语义理解为主，dense_weight 设为 0.8。
</think>
{"intent":"emotion","semantic_query":"无语、无奈、嫌弃的情绪表情包，翻白眼、面无表情、一脸嫌弃的样子，对某事无话可说不想理会，表达对某人某事的无奈和不屑","keywords":["无语","无奈","嫌弃"],"synonyms":["翻白眼","不想说话","懒得理"],"strategy":{"dense_weight":0.8,"need_exact_match":false}}

输入: 熊猫头无语
<think>
查询包含两个部分："熊猫头"是主体类型（subject），"无语"是情绪（emotion），这是复合意图。需要在 filters 中限定熊猫头类别，同时语义描述要结合无语情绪。策略上均衡一些，dense_weight 设为 0.6。
</think>
{"intent":"composite","semantic_query":"熊猫头表情包，表达无语、无奈、嫌弃的情绪，黑白熊猫脸露出一脸嫌弃翻白眼的样子，对某事无话可说","keywords":["熊猫头","无语","无奈"],"synonyms":["嫌弃","翻白眼"],"strategy":{"dense_weight":0.6,"need_exact_match":false},"filters":{"categories":["熊猫头"]}}

输入: 有666的表情包
<think>
用户想找包含"666"文字的表情包，这是 text 意图。需要搜索 OCR 文本，BM25 为主，dense_weight 设为 0.3。关键词就是"666"。
</think>
{"intent":"text","semantic_query":"包含666文字的表情包，写着666、厉害、牛逼的意思，表达赞叹佩服的表情","keywords":["666","厉害","牛"],"synonyms":[],"strategy":{"dense_weight":0.3,"need_exact_match":true}}

现在请理解以下查询：
```

### 4.4 Few-shot 示例设计

每个意图类型至少提供 1 个示例（含思考过程）：

| 查询 | Intent | 思考要点 |
|------|--------|----------|
| "无语" | emotion | 扩展情绪的多种表现形式，dense_weight=0.8 |
| "熊猫头" | subject | 精确匹配 + 分类过滤，dense_weight=0.5 |
| "芭比Q了" | meme | 同义词扩展 (完蛋、凉了)，dense_weight=0.6 |
| "上班摸鱼" | scene | 理解场景下的情绪，dense_weight=0.8 |
| "比心" | action | 动作描述，dense_weight=0.7 |
| "有666的表情包" | text | BM25 优先搜索 OCR，dense_weight=0.3 |
| "熊猫头无语" | composite | 多意图平衡，dense_weight=0.6 |

### 4.5 关键 Prompt 技巧

**1. 思考过程指导**
```
<think> 内容要求:
- 2-4 句话，简洁明了
- 识别查询的核心意图
- 说明为什么选择这个 intent
- 解释 dense_weight 的选择理由
- 前端会实时展示，避免过于技术化的表述
```

**2. 语义改写指导**
```
semantic_query 要求:
- 长度 50-100 字
- 包含原始意图的多种表现形式
- 包含可能的视觉描述 (表情、动作、姿态)
- 不要添加无关内容
```

**3. 关键词提取指导**
```
keywords 要求:
- 提取查询中的核心词
- 包含可能的 OCR 文本
- 包含主体类型词 (如果有)
- 不超过 5 个关键词
```

**4. 同义词扩展指导**
```
synonyms 要求:
- 仅对网络梗/情绪词扩展
- 包含梗的实际含义 (芭比Q → 完蛋)
- 包含常见变体 (yyds → 永远的神)
- 不超过 5 个同义词
```

---

## 5. 检索策略映射

### 5.1 从 QueryPlan 到 HybridSearchPlan

```
QueryPlan.Strategy          HybridSearchPlan
─────────────────────────────────────────────────────────
dense_weight: 0.8      →    DenseLimit:  topK * 4
                            SparseLimit: topK * 1

dense_weight: 0.5      →    DenseLimit:  topK * 2.5
                            SparseLimit: topK * 2.5

dense_weight: 0.3      →    DenseLimit:  topK * 1.5
                            SparseLimit: topK * 3.5
─────────────────────────────────────────────────────────
need_exact_match: true →    BM25 查询使用原始 keywords
                            (不做分词处理)

need_exact_match: false →   BM25 查询使用 keywords + synonyms
                            (允许模糊匹配)
```

### 5.2 BM25 查询构建

```
输入: QueryPlan {
  keywords: ["熊猫头", "无语"],
  synonyms: ["无奈", "嫌弃"]
}

构建逻辑:
1. need_exact_match = true:
   BM25Query = "熊猫头 无语"

2. need_exact_match = false:
   BM25Query = "熊猫头 无语 无奈 嫌弃"
```

### 5.3 过滤条件应用

```
如果 QueryPlan.Filters 存在:

1. categories 不为空:
   添加 Qdrant Filter: category IN [...]

2. is_animated 不为空:
   添加 Qdrant Filter: is_animated = true/false
```

---

## 6. 工程实现

### 6.1 流式解析实现

```go
// StreamParser 解析 LLM 流式输出
type StreamParser struct {
    state        parseState
    thinkBuffer  strings.Builder
    jsonBuffer   strings.Builder
    braceCount   int
}

type parseState int

const (
    stateStart parseState = iota
    stateThinking
    stateAwaitingJSON
    stateParsingJSON
    stateComplete
)

// Feed 处理一个 token
// 返回: (thinkingToken, isComplete)
func (p *StreamParser) Feed(token string) (string, bool) {
    var thinkingOutput string
    
    for _, char := range token {
        switch p.state {
        case stateStart:
            p.thinkBuffer.WriteRune(char)
            if strings.HasSuffix(p.thinkBuffer.String(), "<think>") {
                p.state = stateThinking
                p.thinkBuffer.Reset()
            }
            
        case stateThinking:
            p.thinkBuffer.WriteRune(char)
            if strings.HasSuffix(p.thinkBuffer.String(), "</think>") {
                // 移除结束标签
                content := p.thinkBuffer.String()
                content = strings.TrimSuffix(content, "</think>")
                thinkingOutput = content
                p.state = stateAwaitingJSON
                p.thinkBuffer.Reset()
            }
            
        case stateAwaitingJSON:
            if char == '{' {
                p.jsonBuffer.WriteRune(char)
                p.braceCount = 1
                p.state = stateParsingJSON
            }
            
        case stateParsingJSON:
            p.jsonBuffer.WriteRune(char)
            if char == '{' {
                p.braceCount++
            } else if char == '}' {
                p.braceCount--
                if p.braceCount == 0 {
                    p.state = stateComplete
                    return thinkingOutput, true
                }
            }
        }
    }
    
    // 如果还在 thinking 状态，返回目前累积的内容
    if p.state == stateThinking && thinkingOutput == "" {
        thinkingOutput = token // 直接转发 token
    }
    
    return thinkingOutput, false
}

// GetJSON 获取解析完成的 JSON
func (p *StreamParser) GetJSON() string {
    return p.jsonBuffer.String()
}

// GetThinking 获取思考内容（用于日志）
func (p *StreamParser) GetThinking() string {
    return p.thinkBuffer.String()
}
```

### 6.2 UnderstandStream 实现

```go
func (s *QueryUnderstandingService) UnderstandStream(
    ctx context.Context,
    query string,
    progressCh chan<- UnderstandProgress,
) (*QueryPlan, error) {
    defer close(progressCh)
    
    // 发送开始事件
    progressCh <- UnderstandProgress{
        Stage:   "thinking_start",
        Message: "AI 正在理解搜索意图...",
    }
    
    // 构建请求
    req := s.buildRequest(query, true) // stream=true
    
    // 发送流式请求
    resp, err := s.sendStreamRequest(ctx, req)
    if err != nil {
        return s.fallbackUnderstand(query), nil // Fallback
    }
    defer resp.Body.Close()
    
    // 创建解析器
    parser := &StreamParser{}
    scanner := bufio.NewScanner(resp.Body)
    
    for scanner.Scan() {
        line := scanner.Text()
        if !strings.HasPrefix(line, "data: ") {
            continue
        }
        
        data := strings.TrimPrefix(line, "data: ")
        if data == "[DONE]" {
            break
        }
        
        // 解析 delta
        var delta streamDelta
        if err := json.Unmarshal([]byte(data), &delta); err != nil {
            continue
        }
        
        if len(delta.Choices) == 0 {
            continue
        }
        
        token := delta.Choices[0].Delta.Content
        thinkingToken, isComplete := parser.Feed(token)
        
        // 发送思考内容
        if thinkingToken != "" {
            progressCh <- UnderstandProgress{
                Stage:        "thinking",
                ThinkingText: thinkingToken,
                IsDelta:      true,
            }
        }
        
        if isComplete {
            break
        }
    }
    
    // 解析 JSON
    jsonStr := parser.GetJSON()
    if jsonStr == "" {
        return s.fallbackUnderstand(query), nil
    }
    
    var plan QueryPlan
    if err := json.Unmarshal([]byte(jsonStr), &plan); err != nil {
        return s.fallbackUnderstand(query), nil
    }
    
    // 验证并修正
    plan = s.validateAndFix(&plan, query)
    
    // 发送完成事件
    progressCh <- UnderstandProgress{
        Stage:   "done",
        Message: "理解完成",
    }
    
    return &plan, nil
}
```

### 6.3 Fallback 机制

当 LLM 调用失败时，回退到基于规则的理解：

```go
func (s *QueryUnderstandingService) fallbackUnderstand(query string) *QueryPlan {
    // 基于规则的简单分类（复用现有 Query Routing 逻辑）
    intent := "semantic"
    denseWeight := float32(0.7)
    
    // 检测情绪词
    for _, word := range EmotionWords {
        if strings.Contains(query, word) {
            intent = "emotion"
            denseWeight = 0.8
            break
        }
    }
    
    // 检测主体词
    for _, subject := range []string{"熊猫头", "蘑菇头", "猫咪", "柴犬"} {
        if strings.Contains(query, subject) {
            if intent == "emotion" {
                intent = "composite"
                denseWeight = 0.6
            } else {
                intent = "subject"
                denseWeight = 0.5
            }
            break
        }
    }
    
    return &QueryPlan{
        Intent:        QueryIntent(intent),
        SemanticQuery: query, // 直接使用原始查询
        Keywords:      []string{query},
        Strategy: SearchStrategy{
            DenseWeight:    denseWeight,
            NeedExactMatch: len([]rune(query)) <= 4,
        },
    }
}
```

### 6.4 输出验证

```go
func (s *QueryUnderstandingService) validateAndFix(plan *QueryPlan, originalQuery string) *QueryPlan {
    // 1. 验证 Intent
    validIntents := map[QueryIntent]bool{
        "emotion": true, "meme": true, "subject": true,
        "scene": true, "action": true, "text": true, "composite": true,
    }
    if !validIntents[plan.Intent] {
        plan.Intent = "semantic"
    }
    
    // 2. 验证 DenseWeight 范围
    if plan.Strategy.DenseWeight < 0 {
        plan.Strategy.DenseWeight = 0
    }
    if plan.Strategy.DenseWeight > 1 {
        plan.Strategy.DenseWeight = 1
    }
    
    // 3. 验证 SemanticQuery 长度
    if len([]rune(plan.SemanticQuery)) < 10 {
        plan.SemanticQuery = originalQuery
    }
    if len([]rune(plan.SemanticQuery)) > 200 {
        plan.SemanticQuery = string([]rune(plan.SemanticQuery)[:200])
    }
    
    // 4. 验证 Keywords
    if len(plan.Keywords) == 0 {
        plan.Keywords = []string{originalQuery}
    }
    if len(plan.Keywords) > 5 {
        plan.Keywords = plan.Keywords[:5]
    }
    
    // 5. 验证 Synonyms
    if len(plan.Synonyms) > 5 {
        plan.Synonyms = plan.Synonyms[:5]
    }
    
    return plan
}
```

---

## 7. 未来扩展

### 7.1 多轮对话支持

```
未来可以支持上下文理解:

用户: "熊猫头"
系统: [返回熊猫头表情包]
用户: "无语的"
系统: [理解为 "熊猫头无语"，而不是单独的 "无语"]
```

### 7.2 个性化理解

```
结合用户历史:
- 用户偏好的主体类型
- 用户常用的情绪词
- 调整 dense_weight 偏好
```

### 7.3 多模态查询

```
支持图片输入:
用户: [上传一张表情包图片] "类似这种"
系统:
  1. 用 VLM 理解图片内容
  2. 提取关键特征
  3. 生成 QueryPlan
```

### 7.4 查询建议

```
基于 QueryPlan 生成相关查询建议:

用户: "无语"
QueryPlan.intent = "emotion"
建议: ["无奈", "嫌弃", "翻白眼", "熊猫头无语"]
```

---

## 8. 参考资料

- [Qdrant Hybrid Search](https://qdrant.tech/documentation/concepts/hybrid-queries/)
- [Query Understanding in Search](https://www.pinecone.io/learn/query-understanding/)
- [OpenAI JSON Mode](https://platform.openai.com/docs/guides/structured-outputs)
